...
class TestChatRouter(unittest.TestCase):
    ...
    def test_chat_stream_success(self, mock_config, mock_session_manager):
        ...
        mock_chat_session.chat.return_value = [
            ...
            ConductorResponse(
                ConductorResponseType.FINAL_RESPONSE, "SELECT * FROM alpha"
            ),
    ...
    def test_chat_missing_message_returns_400(self, mock_session_manager):
        ...
        response = self.client.post(
            "/chat/", json={"chat_id": "session_001", "dataset_name": "test_dataset"}
        )
    ...
    def test_list_sessions_delegates_to_search_when_query_provided(
        self, mock_pneuma_db
    ):
    ...
    def test_delete_chat_session_success(
        self, mock_pneuma_db, mock_session_manager, mock_rmtree
    ):
        ...
        mock_session_manager.evict_chat_session.assert_called_once_with(
            "user_999", "session_del"
        )
    ...
    def test_delete_chat_session_not_found_returns_404(self, mock_pneuma_db):
        ...
        mock_pneuma_db.prepare_chat_deletion.side_effect = FileNotFoundError(
            "not found"
        )
...
class TestExplainScriptEndpoint(unittest.TestCase):
    ...
    def _make_session(self, script: str = "print('hello')", description: str = ""):
        from pneuma_seeker.services.core.conductor.state import ConductorState

        mock_chat_session = MagicMock()
        state = ConductorState()
        state.S = script
        state.s_description = description
        mock_chat_session.conductor.state = state
        mock_chat_session.conductor.db_api = MagicMock()
        mock_chat_session.language_model_api.chat.return_value = "A nice explanation."
    ...
    def test_explain_script_calls_llm_when_empty(self, mock_session_manager):
        ...
        response = self.client.get("/chat/explain_script/session_001")
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.json()["description"], "A nice explanation.")
...
class TestDownloadTableEndpoint(unittest.TestCase):
    ...
    def test_download_table_streams_multiple_chunks(self, mock_session_manager):
        ...
        mock_conductor.db_api.execute_query.side_effect = [
            pd.DataFrame(
                {"id": list(range(chunk_size))}
            ),  # full chunk → another call expected
