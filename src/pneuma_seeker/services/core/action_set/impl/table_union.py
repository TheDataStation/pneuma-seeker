...
class TableUnion(Action, Applicable):
    action_name = ActionNames.TABLE_UNION
    ...
    def apply(self, input: dict[str, Any]) -> DataFrame:
        ...
        if (
            ...
            or not provenance_column_name.strip()
        ):
            raise ValueError(
                "Input 'provenance_column_name' must be a non-empty string."
            )
        ...
        tables: list[tuple[str, str, list[str]]] = (
            []
        )  # (display_name, table_ref, columns)
            ...
            for c in all_columns:
                if c in cols_set:
                    projected_cols.append(
                        f"t.{self.__quote_ident(c)} AS {self.__quote_ident(c)}"
                    )
                else:
        ...
        for _, r in rows.iterrows():
            ...
            if catalog == "main" and schema == "main":
                ...
                results.append(
                    (
                        f"{catalog}.{name}",
                        f"{self.__quote_ident(catalog)}.{self.__quote_ident(name)}",
                    )
                )
        return results
        ...
        for item in table_ids:
            ...
            if is_regex:
                ...
                matches = [
                    ...
                    if compiled.search(display)
                    or compiled.search(display.split(".")[-1])
                ]
    ...
    def __extract_provenance_value(
        self, prov_re: re.Pattern[str], display_name: str
    ) -> str:
