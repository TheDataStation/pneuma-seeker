...
class Materializer:
    ...
    def _handle_situational_analysis(self, action_args: dict[str, Any]) -> None:
    ...
    def _handle_table_retrieve(self, action_args: dict[str, Any]) -> None:
    ...
    def _handle_web_search(self, action_args: dict[str, Any]) -> None:
    ...
    def _handle_web_crawl(self, action_args: dict[str, Any]) -> None:
    ...
    def _handle_table_enumeration(self, action_args: dict[str, Any]) -> None:
    ...
    def _handle_table_projection(self, action_args: dict[str, Any]) -> None:
        all_tables = (
            self.state.retrieved_tables
            + self.state.external_tables
            + list(self.state.intermediate_tables)
        ...
        for target_table_id, retrieved_table_info in action_args.items():
            if isinstance(retrieved_table_info, list):
                if len(retrieved_table_info) == 0:
                    ...
                    self.__log(f"==> {msg}")
                    self.llm_messages.append(
                        LLMMessage(role=Role.USER.value, content=msg)
                    )
                    continue
            ...
            if not isinstance(retrieved_table_info, dict):
                ...
                self.__log(f"==> {msg}")
                self.llm_messages.append(LLMMessage(role=Role.USER.value, content=msg))
                continue
    ...
    def _handle_equality_join(self, action_args: dict[str, Any]) -> None:
    ...
    def _handle_table_union(self, action_args: dict[str, Any]) -> None:
    ...
    def _handle_semantic_column_generation(self, action_args: dict[str, Any]) -> None:
    ...
    def _handle_semantic_join(self, action_args: dict[str, Any]) -> None:
        all_tables = (
            self.state.retrieved_tables
            + self.state.external_tables
            + list(self.state.intermediate_tables)
    ...
    def _handle_python_executor(self, action_args: dict[str, Any]) -> None:
        all_tables = (
            self.state.retrieved_tables
            + self.state.external_tables
            + list(self.state.intermediate_tables)
    ...
    def _handle_query_executor(self, action_args: dict[str, Any]) -> None:
        all_tables = (
            self.state.retrieved_tables
            + self.state.external_tables
            + list(self.state.intermediate_tables)
    ...
    def _handle_context_extraction(self, action_args: dict[str, Any]) -> None:
    ...
    def _handle_entity_resolution(self, action_args: dict[str, Any]) -> None:
    ...
    def __reset_materializer(self, update_mode: bool = False):
        ...
        """
        self.__log(f"Resetting materializer (update_mode={update_mode})...")
