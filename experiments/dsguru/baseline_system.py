...
class BaselineLLMSystem(System):
    ...
    def __init__(self, model: str, name="baseline", *args, **kwargs):
        ...
        self.output_dir = kwargs.get(
            "output_dir", os.path.join(os.getcwd(), "testresults")
        )
        ...
        try:
            ...
            for step in answer:
                if isinstance(step, list) or isinstance(step, str):
                    print(
                        f"ERROR: {self.name}: ** ERRORS ** answer is not a dict: {step}"
                    )
                    return self._format_answer_on_fail(
                        answer, f"** ERRORS ** answer is not a dict: {step}"
                    )
    ...
    @profile
    def run_few_shot(
        self, query: str, query_id: str, subset_files: list[str] = []
    ) -> dict[str, str | dict | list | int | float]:
        """
        ...
        prompt_generation_time = time() - start
        ...
        for try_number in range(self.max_tries):
            print(f"=> LLM try number {try_number + 1}...")
            messages.append({"role": "user", "content": prompt})
            # Get the model's response
            start = time()
            response = self.llm(messages)
            total_llm_time += time() - start
            ...
            if self.verbose:
                ...
                print(f"{self.name}: Input token count: {token_count_input}")
                print(f"{self.name}: Output token count: {token_count_output}")
            # Save the token count to a file
            token_fp = os.path.join(
                self.question_output_dir,
                "_intermediate",
                f"token_count-{try_number}.json",
            )
            ...
            print(f"=> Code executed. Output saved to {output_fp}, errors saved to {error_fp}")
        ...
        output_dict["total_llm_time"] = total_llm_time
        output_dict["prompt_generation_time"] = prompt_generation_time
        return output_dict
    ...
    def serve_query(
        self, query: str, query_id: str = "default_name-0", subset_files: list[str] = []
    ) -> dict:
        """
        ...
        output_dict["total_computation_time"] = time() - computation_start
        return output_dict
